"""Open coauthor's local web forms inside Academic Studio instead of the system browser.
...
Today that extension only watches ``~/.voiceover``, so coauthor publishes there when
that channel is live. ``~/.academic-studio`` is checked first: it is the generic
channel a future Studio build should use, and coauthor will prefer it automatically
...
Kept deliberately in sync with ``litdb/inapp.py``; the two plugins ship separately,
so this is a copy rather than a shared import.
"""
...
def publish_url(url: str) -> bool:
    ...
    browser), False when no editor is listening. ``COAUTHOR_NO_BROWSER`` forces True
    without publishing anywhere, for headless runs and tests.
    """
    if os.environ.get("COAUTHOR_NO_BROWSER"):
        return True
