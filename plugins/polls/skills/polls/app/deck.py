"""Reading and checking a poll file.
...
def load(path: str | Path) -> dict[str, Any]:
    ...
    if not target.is_file():
        raise DeckError(f"No poll file at {target}")
    try:
        raw = json.loads(target.read_text())
    except json.JSONDecodeError as exc:
        raise DeckError(f"{target} is not valid JSON: {exc}") from exc

    deck = normalise(raw)
    deck["path"] = str(target)
    if not deck["title"]:
        deck["title"] = target.stem.replace("-", " ").replace("_", " ")
    return deck
